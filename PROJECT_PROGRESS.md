...
## Completed
- [SAM_Origin#6](https://github.com/SAM-BIM/SAM_Origin/pull/6) merged as `9a9070a3`: removed dead .NET Framework `app.config` files.
...
## Files changed
- `Grasshopper/SAM.Analytical.Grasshopper.Origin/app.config` (deleted)
- `SAM_Origin/SAM.Analytical.Origin/app.config` (deleted)
...
- CI on the PR: none (repo has no workflows) - covered by the local BuildAlls only.
